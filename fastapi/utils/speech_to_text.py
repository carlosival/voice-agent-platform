...
async def call_stt(http_client: AsyncClient, audio: bytes, is_wav: bool = False) -> str:
    """
...
# ─── How to use in _process_audio() ──────────────────────────────────────────
#
# from state import app_state
#
# async def _process_audio(track, output_track, ws):
#     frames      = []
#     frame_count = 0
#     CHUNK_FRAMES = 150    # ~3 seconds of audio (150 × 20ms)
#
#     while True:
#         try:
...
#                 text = await call_stt_from_frames(app_state.http, frames)
#                 frames = []   # reset buffer
#
#                 if text:
#                     logger.info(f"STT: {text}")
#                     asyncio.ensure_future(
#                         handle_pipeline(text, output_track, ws)
#                     )
#
#         except MediaStreamError:
#             # Transcribe any remaining frames on disconnect
#             if frames:
#                 text = await call_stt_from_frames(app_state.http, frames)
#                 if text:
